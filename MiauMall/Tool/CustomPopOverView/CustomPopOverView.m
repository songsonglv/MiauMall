...
- (instancetype)init
{
    ...
    if (self) {
        ...
        _containerBorderColor = [UIColor colorWithRed:(245/255.0) green:(245/255.0) blue:(245/255.0) alpha:1];
        _shadowColor = [UIColor colorWithRed:(180/255.0) green:(180/255.0) blue:(180/255.0) alpha:0.15];
        _containerBackgroudColor = [UIColor colorWithRed:(255/255.0) green:(255/255.0) blue:(255/255.0) alpha:1];
        _containerCornerRadius = 2.5f;
        _shouldDismissOnTouchOutside = YES;
        _isNeedAnimate = YES;
        _animationDuration = 0.2;
        
        // 普通用法
        _defaultRowHeight = 37.f;
        ...
        _textColor = TCUIColorFromRGB(0x1d1d1d);
...
- (void)setLayerFrame:(CGRect)frame
{
    ...
    if (_style.shadowColor) {
        ...
        self.popLayer.shadowOffset = CGSizeMake(0, 5);
        self.popLayer.shadowOpacity = 1;
...
- (void)updateSubViewFrames
{
    ...
    switch (_alignStyle) {
        ...
        {
            CGRect frame = self.containerView.frame;
            frame.origin.x = CGRectGetMinX(newFrame) - (fabs(frame.size.width - newFrame.size.width)) - 30;
            self.containerView.frame = frame;
            
            self.containerView.apexOftriangelX = CGRectGetWidth(self.containerView.frame) - CGRectGetWidth(_showFrom.frame)/2 + 30;
        }
...
- (UITableView *)table
{
    if (nil == _table) {
        ...
        _table.backgroundColor = TCUIColorFromRGB(0xffffff);
