...
-(void)setUI{
    UILabel *titleLa = [UILabel publicLab:@"凑单免运费" textColor:TCUIColorFromRGB(0xa5844d) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-SemiBold" size:12 numberOfLines:0];
    ...
    [btn setTitle:@"去凑单" forState:(UIControlStateNormal)];
...
-(void)setModel:(MMShopCarModel *)model{
    _model = model;
    self.contLa.text = _model.freefreightStr;
    CGSize size = [NSString sizeWithText:_model.freefreightStr font:[UIFont fontWithName:@"PingFangSC-Medium" size:12] maxSize:CGSizeMake(WIDTH - 24,MAXFLOAT)];
    if(CGRectGetMaxX(self.titleLa.frame) + 12 + size.width > self.couBt.x){
        self.contLa.frame = CGRectMake(12, 24, WIDTH - 24, size.height);
    }else{
