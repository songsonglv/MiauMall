...
-(void)creatUI{
    ...
    if([self.type isEqualToString:@"0"]){
        arr = @[@"商品金额",@"运费",@"折扣码",@"优惠券",@"蜜豆"];
    }else{
        arr = @[@"商品金额",@"运费",@"折扣码",@"优惠券",@"蜜豆",@"赠言"];
    }
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        if(i == 0){
            ...
            field.attributedPlaceholder = [[NSAttributedString alloc]initWithString:@"请输入折扣码" attributes:@{NSForegroundColorAttributeName:TCUIColorFromRGB(0xa9a9a9)}];
            field.delegate = self;
    ...
    UILabel *tipsLa = [UILabel publicLab:@"注意优惠券与折扣码不可同时叠加使用" textColor:TCUIColorFromRGB(0xa5844d) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
...
-(void)setModel:(MMConfirmOrderModel *)model{
    ...
    self.myIntegralLa.text = [NSString stringWithFormat:@"蜜豆（共%@个）",_model.MyIntegral];
