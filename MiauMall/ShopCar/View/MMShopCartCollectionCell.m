...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
        ...
        if([model.ProductSign isEqualToString:@"品牌直供"]){
            str = @"品牌直供";
            size = [NSString sizeWithText:@"品牌直供" font:[UIFont fontWithName:@"PingFangSC-Medium" size:15] maxSize:CGSizeMake(MAXFLOAT,15)];
        }else if ([model.OptionalID isEqualToString:@"0"]){
            str = @"自营";
            size = [NSString sizeWithText:@"自营" font:[UIFont fontWithName:@"PingFangSC-Medium" size:15] maxSize:CGSizeMake(MAXFLOAT,15)];
        }
    ...
    if([model.OptionalID isEqualToString:@"0"]){
        ...
        UILabel *lab1 = [UILabel publicLab:@"去凑单" textColor:TCUIColorFromRGB(0x919191) textAlignment:(NSTextAlignmentRight) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
...
-(UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UIContextualAction *similarRowAction = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:@"差找相似" handler:^(UIContextualAction * _Nonnull action, __kindof UIView * _Nonnull sourceView, void (^ _Nonnull completionHandler)(BOOL)) {
        ...
        NSArray *arr = self.allArr[indexPath.section];
        MMShopCarGoodsModel *model = arr[indexPath.row];
        weakself.tapSimilarBlock(model);
    }];
    similarRowAction.title = @"差找相似";
