...
//        layout.sectionHeadersPinToVisibleBounds = NO;
    ...
    layout.headerReferenceSize = CGSizeMake(WIDTH - 80,44);
...
-(UITableView *)leftTableView{
    if(!_leftTableView){
        ...
        _leftTableView.backgroundColor = TCUIColorFromRGB(0xf6f6f6);
...
- (void)viewDidLoad {
    ...
    UILabel *lab = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"isearch"] textColor:TCUIColorFromRGB(0x565656) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
    ...
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"iclassifi"],[UserDefaultLocationDic valueForKey:@"brandMuseum"]];
    CGFloat wid = WIDTH/2;
...
-(void)requestData{
    //GetClassPageNew
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetClassPageNew"];
...
//            [weakself.leftArr insertObject:model atIndex:0];
            ...
            MMSortDataModel *model1 = arr[0];
            NSArray *arr1 = [MMSortDataModel mj_objectArrayWithKeyValuesArray:model1.child];
            weakself.classifyArr = [NSMutableArray arrayWithArray:arr1];
            [weakself setUI];
        }
...
//    else if (tableView == self.rightTableView){
//        MMSortDataModel *model = self.dataArr[section];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(tableView == self.leftTableView){
        ...
        if (self.dataArr.count > 0) {
            MMSortDataModel *model = self.dataArr[indexPath.row];
            cell.model = model;
        }
...
-(NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section{
    MMSortDataModel *model = self.classifyArr[section];
    NSArray *arr = [MMSortDataModel mj_objectArrayWithKeyValuesArray:model.child];
    return arr.count;
}
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    KweakSelf(self);
    MMSortDataModel *model = self.classifyArr[indexPath.section];
    ...
    if(kind == UICollectionElementKindSectionHeader){
        ...
        UIView *bgView = [[UIView alloc]initWithFrame:CGRectMake(10, 10, WIDTH - 100, 34)];
        bgView.backgroundColor = TCUIColorFromRGB(0xf6f6f6);
        bgView.layer.masksToBounds = YES;
        bgView.layer.cornerRadius = 3;
        [headV addSubview:bgView];
        
        UILabel *lab1 = [UILabel publicLab:model.name textColor:TCUIColorFromRGB(0x000000) textAlignment:(NSTextAlignmentRight) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        lab1.preferredMaxLayoutWidth = 150;
        [lab1 setContentHuggingPriority:(UILayoutPriorityRequired) forAxis:(UILayoutConstraintAxisHorizontal)];
