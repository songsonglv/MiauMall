...
//        make.left.mas_equalTo(7);
//        make.top.mas_equalTo(weakself.goodsNameLa.mas_bottom).offset(9);
...
//    }];
    ...
    [self.flatMoneyLa mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(weakself.deliverLa.mas_bottom).offset(17);
    ...
    [self.priceLa mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.mas_equalTo(weakself.deliverLa.mas_bottom).offset(13);
...
-(void)setModel1:(MMHomeGoodsModel *)model1{
    ...
    UIColor *col = [_model1.ColumnID isEqualToString:@"828"] ? TCUIColorFromRGB(0x333333) : redColor2;
    self.tagLa.text = _model1.ProductSign;
    self.tagLa.backgroundColor = col;
    
    if(_model1.ProductSign){
        self.tagLa.hidden = NO;
    }else{
        self.tagLa.hidden = YES;
    }
    ...
    NSAttributedString *attrText = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",_model1.name] attributes:@{NSParagraphStyleAttributeName: style}];
    if([_model1.ColumnID isEqualToString:@"828"]){
        style.firstLineHeadIndent = size1.width + 12;//首行缩进
        attrText = [[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%@",_model.Name] attributes:@{NSParagraphStyleAttributeName: style}];
    }
    ...
    NSInteger realnum = [_model1.realnumber integerValue];
    if(realnum > 0){
        self.deliverView.hidden = NO;
    }else{
        self.deliverView.hidden = YES;
    ...
    if(discount > 0){
        self.discountLa.text = [NSString stringWithFormat:@"%.f折",discount * 10];
    }else{
        self.discountLa.hidden = YES;
    }
    
//    self.flatMoneyLa.text = _model1.pricesign;
    self.priceLa.text = _model1.priceshow;
    //self.salesLa.text = [NSString stringWithFormat:@"销量%@",_model1.Sales];
