...
-(void)setScroll{
    ...
    for (int i = 0; i < self.todayArr.count; i++) {
        ...
        UILabel *goLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"iClaimed"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        ...
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 110, 208)];
        btn.tag = 100 + i;
...
-(void)setScroll1{
    ...
    for (int i = 0; i < self.tomorrowArr.count; i++) {
        ...
        UILabel *goLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"tobegin"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        goLa.frame = CGRectMake(0, 10, 100, 14);
        [bgImage addSubview:goLa];
...
-(void)clickBt:(UIButton *)btn{
    ...
    MMHomeLimitGoodsModel *model = self.todayArr[tag];
    self.goodsRouterBlock(model.ID);
    
}
-(void)clickBt1:(UIButton *)btn{
    NSInteger tag = btn.tag - 200;
    MMHomeLimitGoodsModel *model = self.tomorrowArr[tag];
    self.goodsRouterBlock(model.ID);
