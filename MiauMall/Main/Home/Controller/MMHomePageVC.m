...
@interface MMHomePageVC ()<UITextFieldDelegate,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout,HGCategoryViewDelegate,WXApiDelegate>
{
...
@property (nonatomic, strong) NSMutableArray *limit8Arr;//限时抢购数组

@property (nonatomic, strong) NSString *isEvaluate;//是否有好评有礼装修
@property (nonatomic, strong) NSString *ysTime;//好评有礼弹窗延时弹出时间几秒
@property (nonatomic, strong) NSString *ysday;//好评有礼弹窗几天弹出一次
@property (nonatomic, strong) NSMutableArray *idsArr;//弹窗上的文字数组
@property (nonatomic, strong) MMEvaluatePolitelyPopView *evalutePopView;
...
@property (nonatomic, strong) MMVersionUpdatePopView *uploadPopView;
@property (nonatomic, strong)MMVersionModel *versionModel;//版本model
...
//    [kNotificationCenter addObserver:self selector:@selector(changeLauange) name:@"selectCountry" object:nil];
...
//    });
    ...
    dispatch_group_async(group, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        //请求底部数据
        [weakself GetStoreRecommendAndTypeID:self.storeID AndIsRec:self.isRec];
    });
...
-(void)requestPopText{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetTipsByID"];
    NSDictionary *param = @{@"ids":@"421,422,423,424,425",@"lang":self.lang,@"cry":self.cry};
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        NSLog(@"%@",jsonDic);
        NSArray *arr = jsonDic[@"IDTips"];
        weakself.idsArr = [NSMutableArray arrayWithArray:arr];
        double ys = [weakself.ysTime doubleValue];
        [self performSelector:@selector(evaPopView) withObject:nil afterDelay:ys];
...
-(void)evaPopView{
    KweakSelf(self);
    self.evalutePopView = [[MMEvaluatePolitelyPopView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT) andTextArr:self.idsArr];
    self.evalutePopView.clickGoBlock = ^(NSString * _Nonnull str) {
        ...
        if(weakself.memberToken){
            MMEvalutePolitelyViewController *evaVC = [[MMEvalutePolitelyViewController alloc]init];
    ...
    self.evalutePopView.goTcBlock = ^(NSString * _Nonnull str) {
        if(weakself.memberToken){
    ...
    [[UIApplication sharedApplication].keyWindow addSubview:self.evalutePopView];
    [self.evalutePopView showView];
}
...
-(void)setUI{
    ...
    double appVersion = [applocalversion floatValue];
    double iosVersion = [self.versionModel.IOSVersion floatValue];
    //appVersion >= iosVersion ||
    if( appVersion >= iosVersion || [self.versionModel.IsOpenIOS isEqualToString:@"0"]){
        
    }else{
        self.uploadPopView = [[MMVersionUpdatePopView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT) andModel:self.versionModel];
        self.uploadPopView.clickGoBlock = ^(NSString * _Nonnull str) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"https://apps.apple.com/us/app/miaumall-%E6%97%A5%E6%9C%AC%E7%9B%B4%E9%82%AE%E5%A5%BD%E5%B9%B3%E5%8F%B0/id1562454331"]];
        };
        [[UIApplication sharedApplication].keyWindow addSubview:self.uploadPopView];
        [self.uploadPopView showView];
    }
    
    
    
    if([self.isEvaluate isEqualToString:@"1"]){
//        NSInteger ys = [self.ysTime integerValue];
        NSInteger day = [self.ysday integerValue];
        
        NSString *showDay = [self.userDefaults valueForKey:@"ShowDay"];
        if(!showDay){
            [self requestPopText];
            NSDate *datenow = [NSDate date];//现在时间
            NSString *timeSp = [NSString stringWithFormat:@"%ld", (long)([datenow timeIntervalSince1970])];//时间戳 秒
            [self.userDefaults setValue:timeSp forKey:@"ShowDay"];
            [self.userDefaults synchronize];
        }else{
            NSDate *datenow = [NSDate date];//现在时间
            NSString *timeSp = [NSString stringWithFormat:@"%ld", (long)([datenow timeIntervalSince1970])];//时间戳 秒
            NSInteger time = [showDay integerValue];
            NSInteger time1 = [timeSp integerValue];
            if(time1 >= time + day * 24 * 3600){
                [self requestPopText];
                [self.userDefaults setValue:timeSp forKey:@"ShowDay"];
                [self.userDefaults synchronize];
            }
    ...
    [notifiBt setEnlargeEdgeWithTop:10 right:10 bottom:10 left:10];
...
-(void)GetVersion{
    ...
    [param setValue:applocalversion forKey:@"AppVersion"];
...
-(void)GetStores{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            if(weakself.categoryView){
                NSMutableArray *arr = [NSMutableArray array];
                for (int i = 0; i < self.storeArr.count; i++) {
                    MMSortModel *model = weakself.storeArr[i];
                    [arr addObject:model.Name];
                }
            }
        }
...
//        NSInteger sectionToReload = 1; // 需要刷新的分区
//        [weakself.mainCollectionView performBatchUpdates:^{
//            [weakself.mainCollectionView reloadSections:[NSIndexSet indexSetWithIndex:sectionToReload]];
//        } completion:nil];
...
//            dispatch_semaphore_signal(sema);
            ...
            if([codeStr isEqualToString:@"1"]){//请求成功
                ...
                for (MMHomePageItemModel*itemModel in items) {
                    ...
                    }else if ([itemModel.type isEqualToString:@"middlepop"]){
                        weakself.isEvaluate = @"1";
                        weakself.ysTime = itemModel.defin4;
                        weakself.ysday = itemModel.defin3;
                    }
                ...
                dispatch_async(q1, ^{
                    ...
                    //单图高度
//                    [weakself.singleHArr removeAllObjects];
                    ...
                    //双图1高度
//                    [weakself.twoimg1HArr removeAllObjects];
                    ...
                    //双图组合2高度
//                    [weakself.twoimg2HArr removeAllObjects];
                    ...
                    //两图切换高度
//                    [weakself.imageTwoturnHArr removeAllObjects];
                    ...
                    //三图三高度
//                    [weakself.threeImgThreeHArr removeAllObjects];
                    ...
                    //三图4高度
//                    [weakself.threeImg4HArr removeAllObjects];
                    ...
                    //九图轮播高度
//                    [weakself.nineHArr removeAllObjects];
                    ...
                    //堆叠轮播高度
//                    [weakself.stackbannerHArr removeAllObjects];
                    ...
                    //tab分类高度
//                    [weakself.tabHArr removeAllObjects];
...
-(void)categoryViewDidSelectedItemAtIndex:(NSInteger)index{
    ...
    [self.mainCollectionView setContentOffset:CGPointMake(0, self.scrollY)];
//    [self.mainCollectionView setContentOffset:CGPointMake(0, self.hei)];
...
-(void)clickScan{
//    MMScanViewController *scanVC = [[MMScanViewController alloc]init];
...
//    [self RouteJump:@"/pages/index/page?id=1145"];
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if(self.mainCollectionView.contentOffset.y >= self.hei){
        self.scrollY = self.hei;
    }else{
        self.scrollY = self.mainCollectionView.contentOffset.y;
    }
