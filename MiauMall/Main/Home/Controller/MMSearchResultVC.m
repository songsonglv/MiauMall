...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"proList"];
    ...
    NSString *isrec = self.param[@"isrec"];
    if(isrec){
        self.isrec = isrec;
        [self GetProductScreen];
        [self requestRouterSearchDataPage];
    }else{
...
-(void)SetSearchRecord{
    ...
    NSDictionary *param1 = @{@"location":@"0"};
...
-(void)setUI{
    ...
    self.branPopView.sureSelectBlock = ^(NSArray * _Nonnull seleArr, NSString * _Nonnull nameStr) {
        ...
        if([weakself.branPopView.type isEqualToString:@"0"]){
            ...
            if(seleArr.count > 0){
                ...
                [weakself.sortBt setImage:[UIImage imageNamed:@"triangle_down_black"] forState:(UIControlStateNormal)];
                [weakself.sortBt setTitle:[UserDefaultLocationDic valueForKey:@"iclassifi"] forState:(UIControlStateNormal)];
                [weakself.sortBt layoutButtonWithEdgeInsetsStyle:(GLButtonEdgeInsetsStyleRight) imageTitleSpace:5];
            }
        }else{
    ...
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"speed48h"],[UserDefaultLocationDic valueForKey:@"salesVol"],[UserDefaultLocationDic valueForKey:@"iprice"]];
    CGFloat wid = WIDTH/3;
    CGFloat space = 0;
    ...
    NSArray *arr1 = @[[UserDefaultLocationDic valueForKey:@"iclassifi"],[UserDefaultLocationDic valueForKey:@"ibrand"],[UserDefaultLocationDic valueForKey:@"xinping"]];
...
//路由进来的
-(void)requestRouterSearchDataPage{
    KweakSelf(self);
    NSMutableDictionary *param = [[NSMutableDictionary alloc]init];
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetProductPageList"];
    if(self.memberToken){
...
//    [self.mainCollectionView reloadData];
    ...
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetProductPageList"];
    if([self.pageNum isEqualToString:@"1"]){
        url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetProductPageList"];
