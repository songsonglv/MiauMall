...
@property (nonatomic, strong) NSMutableArray *contStrArr;
@property (nonatomic, strong) NSMutableArray *contStrArr2;
@end

@implementation MMNewUserOrderVC

-(NSMutableArray *)contStrArr{
    if(!_contStrArr){
        _contStrArr = [NSMutableArray array];
    }
    return _contStrArr;
}

-(NSMutableArray *)contStrArr2{
    if(!_contStrArr2){
        _contStrArr2 = [NSMutableArray array];
    }
    return _contStrArr2;
}
...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"orderPolitely"];
...
-(void)GetTreeOrder{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            NSArray *str1 = jsonDic[@"ContsStr"];
            weakself.contStrArr = [NSMutableArray arrayWithArray:str1];
            NSArray *str2 = jsonDic[@"ContsStr2"];
            weakself.contStrArr2 = [NSMutableArray arrayWithArray:str2];
...
-(void)setUI{
    ...
    UILabel *lab1 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"isdyl"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:22 numberOfLines:0];
    ...
    UILabel *lab2 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"ixrsdmfs"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:12 numberOfLines:0];
    ...
    for (int i = 0; i < self.goodsArr.count; i++) {
        ...
        UILabel *goLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"getNow"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Medium" size:13 numberOfLines:0];
    ...
    UILabel *titleLa = [UILabel publicLab:[NSString stringWithFormat:@"- %@ -",[UserDefaultLocationDic valueForKey:@"irule"]] textColor:TCUIColorFromRGB(0x333333) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Regular" size:18 numberOfLines:0];
...
//    NSArray *titleArr = @[@"* 新用户首次下单时，可免费领取一份首单赠礼。",@"* 首单赠礼产品仅限首单领取，首单未领即视作自动放弃。",@"* 首单赠礼与首单其他商品同遵循MiauMall运费政策。",@"* 首单赠礼产品不做退换货处理。"];
    float hei = 0;
    for (int i = 0; i < self.contStrArr.count; i++) {
        NSString *str = self.contStrArr[i];
...
//        lab.frame = CGRectMake(16,67 + hei, size1.width, size1.height);
    ...
    UILabel *lab3 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"isdyl"] textColor:TCUIColorFromRGB(0x333333) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:22 numberOfLines:0];
    ...
    UILabel *lab4 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"ixmsdhlxz"] textColor:TCUIColorFromRGB(0x333333) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:12 numberOfLines:0];
...
//    NSArray *arr = @[@"1",@"2",@"3"];
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"idyd"],[UserDefaultLocationDic valueForKey:@"ided"],[UserDefaultLocationDic valueForKey:@"idsd"]];
    ...
    UILabel *titleLa2 = [UILabel publicLab:[NSString stringWithFormat:@"- %@ -",[UserDefaultLocationDic valueForKey:@"irule"]] textColor:TCUIColorFromRGB(0x333333) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-SemiBold" size:14 numberOfLines:0];
...
//    NSArray *titleArr2 = @[@"新用户在 MiauMall 的前三笔订单均有奖励可领取：",@"*第一单：首次购物时，可在【首单赠礼】挑选任一礼品。确认收货后，可得无门槛1000日币优惠券一张。",@" *第二单：确认收货后，可得无门槛1000日币优惠券一张。",@" *第三单：确认收货后，可得无门槛1000日币优惠券一张。"];
    float hei1 = 0;
    for (int i = 0; i < self.contStrArr2.count; i++) {
        NSString *str = self.contStrArr2[i];
