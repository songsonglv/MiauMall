...
@interface MMSortAndBrandPopView ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
@implementation MMSortAndBrandPopView
...
-(__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.type isEqualToString:@"0"]){
        MMCollageSortAndBrandModel*model = self.dataArr[indexPath.item];
        cell.model2 = self.dataArr[indexPath.item];
        ...
        MMCollageSortAndBrandModel *model = self.dataArr1[indexPath.item];
        cell.model2 = self.dataArr1[indexPath.item];
    ...
    if([self.type isEqualToString:@"0"]){
        for (NSString *ID in self.seleArr) {
            for (int i = 0; i < self.dataArr.count; i++) {
                MMCollageSortAndBrandModel *model = self.dataArr[i];
        ...
        for (NSString *ID in self.seleArr1) {
            for (int i = 0; i < self.dataArr1.count; i++) {
                MMCollageSortAndBrandModel *model = self.dataArr1[i];
                if([model.ID isEqualToString:ID]){
