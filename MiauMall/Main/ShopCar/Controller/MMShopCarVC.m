...
@property (nonatomic, strong) NSString *isPopShow;
...
@property (nonatomic, strong) MMShopCartChangeAttidPopView *skuPopView;
@property (nonatomic, strong) MMShopCarGoodsModel *goodsModel;
@property (nonatomic, strong) MMSelectSKUPopView *skuPopView1;
...
- (void)viewDidLoad {
    ...
    self.lang = [self.userDefaults valueForKey:@"language"];
    self.cry = [self.userDefaults valueForKey:@"cry"];
    self.isPopShow = [self.userDefaults valueForKey:@"PopShow"];
    self.page = 1;
    ...
    UILabel *lab = [UILabel publicLab:[NSString stringWithFormat:@"%@(%@)",[UserDefaultLocationDic valueForKey:@"myCart"],@"0"] textColor:TCUIColorFromRGB(0x383838) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:16 numberOfLines:0];
    ...
    [collectionBt setEnlargeEdgeWithTop:10 right:10 bottom:10 left:10];
    [topV addSubview:collectionBt];
    
    UIButton *editBt = [[UIButton alloc]initWithFrame:CGRectMake(WIDTH - 110, StatusBarHeight + 20, 60, 14)];
    ...
    [editBt setEnlargeEdgeWithTop:10 right:10 bottom:10 left:10];
    ...
    self.popView.selectGoodsBlock = ^(MMShopCarGoodsModel * _Nonnull model, NSString * _Nonnull isBuy) {
...
-(void)requestData{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.numLa.text = [NSString stringWithFormat:@"%@(%@)",[UserDefaultLocationDic valueForKey:@"shopCart"],weakself.model.num];
            ...
            NSArray *arr4 = [MMShopCarGoodsModel mj_objectArrayWithKeyValuesArray:allNumArr];
            for (MMShopCarGoodsModel *model in arr4) {
...
-(__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.footArr.count > 0){
        if(indexPath.section == 0){
            ...
            cell.NoDiscountTips = self.model.NoDiscountTips;
            ...
            cell.tapUpdataBlock = ^(MMShopCarGoodsModel * _Nonnull model, NSString * _Nonnull num) {
                [weakself CartUpdate:model andNum:num];
            };
            ...
            cell.tapLikeBlock = ^(NSString * _Nonnull str, MMShopCarGoodsModel * _Nonnull model) {
                [weakself goFindSimilaryandModel:model];
            };
...
//            };
            ...
            cell.TapCarBlock = ^(NSString * _Nonnull indexStr) {
                ...
                [weakself GetProductBuyTest1:indexStr];
            };
            return cell;
        }
    }else{
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if(self.footArr.count > 0){
        ...
        if(indexPath.section == 0){
            return CGSizeMake(WIDTH, self.hei + 10);
        }else{
            return  CGSizeMake((WIDTH - 30)/2, (WIDTH - 30)/2 + 153);
        }
    }
...
-(void)discoundViewShow{
    ...
    self.popView.model1 = self.model;
...
//更新商品数据
-(void)CartUpdate:(MMShopCarGoodsModel *)model andNum:(NSString *)num{
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"CartUpdate"];
    NSDictionary *param1 = @{@"lang":self.lang,@"cry":self.cry,@"tempcart":self.tempcart,@"id":model.ID,@"attid":model.attid,@"back":@"1",@"num":num};
...
//修改多规格
-(void)CartChange:(NSString *)newAttid andNum:(NSString *)num{
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"CartChange"];
    NSDictionary *param1 = @{@"lang":self.lang,@"cry":self.cry,@"tempcart":self.tempcart,@"id":self.goodsModel.ID,@"oattid":self.goodsModel.attid,@"back":@"1",@"num":num,@"attid":newAttid};
...
-(void)Collection:(MMShopCarGoodsModel *)model{
    ...
    NSDictionary *param1 = @{@"lang":self.lang,@"cry":self.cry,@"ids":model.ID,@"isTrue":@"1"};
...
-(void)BCollection{
    ...
    if(self.seleArr.count == 0){
        ...
        NSDictionary *param1 = @{@"lang":self.lang,@"cry":self.cry,@"ids":ids,@"isTrue":@"1"};
...
-(void)requestUrl:(NSString *)url andParam:(NSDictionary *)param{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.popView.model = weakself.model;
            NSArray *arr = jsonDic[@"key"][@"newitem"];
            weakself.allArr = [NSMutableArray arrayWithArray:arr];
            weakself.numLa.text = [NSString stringWithFormat:@"%@(%@)",[UserDefaultLocationDic valueForKey:@"myCart"],weakself.model.num];
...
-(void)GetProductBuyTest:(NSString *)ID{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.skuPopView = [[MMShopCartChangeAttidPopView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT) andData:weakself.goodsSpecInfo andGoodsModel:weakself.goodsModel];
            ...
            self.skuPopView.SkuAndNumBlock = ^(MMGoodsAttdataModel * _Nonnull model, NSString * _Nonnull numStr) {
                ...
                [weakself CartChange:model.ID andNum:numStr];
            };
...
-(void)GetProductBuyTest1:(NSString *)ID{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetProductBuyTest"];
    NSMutableDictionary *param = [[NSMutableDictionary alloc]init];
    NSDictionary *param1 = @{@"id":ID,@"lang":self.lang,@"cry":self.cry};
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    NSString *popshow = [self.userDefaults valueForKey:@"PopShow"];
    if([popshow isEqualToString:@"1"]){
