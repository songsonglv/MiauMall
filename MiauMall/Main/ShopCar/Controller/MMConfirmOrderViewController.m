...
@property (nonatomic, strong) MMRealNamePopView *realNamePop;
@property (nonatomic, strong) NSString *isRealNamePop;//是否弹 0 否 1 是
@property (nonatomic, strong) NSMutableArray *tipsArr;//备注数组
...
-(UIView *)bottomV{
    if(!_bottomV){
        ...
        UILabel *lab = [UILabel publicLab:[NSString stringWithFormat:@"%@%@(共%@%@)",[UserDefaultLocationDic valueForKey:@"shouldPay"],self.model.PayMoneyShow,self.model.BuyNum,[UserDefaultLocationDic valueForKey:@"ipiece"]] textColor:TCUIColorFromRGB(0x6c6c6c) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
        ...
        [lab rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
            confer.text([NSString stringWithFormat:@"%@: ",[UserDefaultLocationDic valueForKey:@"shouldPay"]]).textColor(TCUIColorFromRGB(0x6c6c6c)).font([UIFont systemFontOfSize:13]);
            ...
            confer.text([NSString stringWithFormat:@"(%@%@%@)",[UserDefaultLocationDic valueForKey:@"ioverall"],self.model.BuyNum,[UserDefaultLocationDic valueForKey:@"ipiece"]]).textColor(TCUIColorFromRGB(0x6c6c6c)).font([UIFont systemFontOfSize:12]);
        }];
        
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(WIDTH - 120, 8, 100, 40)];
        [btn setTitle:[UserDefaultLocationDic valueForKey:@"submitorder"] forState:(UIControlStateNormal)];
...
-(UIView *)couView{
    ...
    if(!_couView){
        ...
        [lab rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
            confer.text([UserDefaultLocationDic valueForKey:@"shipAssistant"]).textColor(TCUIColorFromRGB(0x1e1e1e));
            confer.text(self.model.FreeShipNeedMoneyShow).textColor(redColor2);
            confer.text([UserDefaultLocationDic valueForKey:@"freeShipping"]).textColor(TCUIColorFromRGB(0x1e1e1e));
        }];
        
        CGSize size = [NSString sizeWithText:[UserDefaultLocationDic valueForKey:@"addMore"] font:[UIFont fontWithName:@"PingFangSC-Medium" size:12] maxSize:CGSizeMake(MAXFLOAT,12)];
        UIButton *btn = [[UIButton alloc]init];
        [btn setTitle:[UserDefaultLocationDic valueForKey:@"addMore"] forState:(UIControlStateNormal)];
        [btn setTitleColor:TCUIColorFromRGB(0xffffff) forState:(UIControlStateNormal)];
        [btn setBackgroundColor:redColor2];
...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"confirmOrder"];
...
//是否实名认证
-(void)CheckRealNameAuth{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"CheckRealNameAuth"];
    NSDictionary *param = @{@"membertoken":self.memberToken,@"lang":self.lang,@"cry":self.cry};
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        NSLog(@"%@",jsonDic);
        weakself.realNameModel = [MMRealNameModel mj_objectWithKeyValues:jsonDic];
        if([weakself.realNameModel.IDCard isEqualToString:@""] || [weakself.realNameModel.CardPositive isEqualToString:@""] || [weakself.realNameModel.CardBack isEqualToString:@""]){
            weakself.isRealNamePop = @"1";
        }else if (weakself.model.list2.count > 0 && ([weakself.realNameModel.IDCard2 isEqualToString:@""] || [weakself.realNameModel.CardPositive2 isEqualToString:@""] || [weakself.realNameModel.CardBack2 isEqualToString:@""])){
            weakself.isRealNamePop = @"1";
        }else if (weakself.model.list3.count > 0 && ([weakself.realNameModel.IDCard3 isEqualToString:@""] || [weakself.realNameModel.CardPositive3 isEqualToString:@""] || [weakself.realNameModel.CardBack3 isEqualToString:@""])){
            weakself.isRealNamePop = @"1";
        }
...
-(void)GetTipsByID{
    KweakSelf(self);
    NSDictionary *param1 = @{@"lang":self.lang,@"cry":self.cry,@"ids":@"418"};
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetTipsByID"];
    
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param1 ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        NSLog(@"%@",jsonDic);
        NSArray *arr = jsonDic[@"IDTips"];
        weakself.tipsArr = [NSMutableArray arrayWithArray:arr];
    } failure:^(NSError *error) {
        NSLog(@"%@",error);
    }];
}
...
-(void)setUI{
    ...
    self.realNamePop = [[MMRealNamePopView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT) andData:self.model];
    ...
    for (int i = 0; i < index; i++) {
        NSString *weight = self.model.Weight[i];
        ...
        packageView.tipsArr = self.tipsArr;
...
-(void)clickPay{
    ...
    if(index == 3){
        if(!self.adsid){
            str = [UserDefaultLocationDic valueForKey:@"fillPackage"];
        }else if (!self.adsid2){
            str = [UserDefaultLocationDic valueForKey:@"fillPackage2"];
        }else if (!self.adsid3){
            str = [UserDefaultLocationDic valueForKey:@"fillPackage3"];
        }
    }else if (index == 2){
        if(!self.adsid){
            str = [UserDefaultLocationDic valueForKey:@"fillPackage"];
    ...
    if(str.length > 0){
        ...
        if([self.isRealNamePop isEqualToString:@"1"]){
            MMRealNameViewController *realNameVC = [[MMRealNameViewController alloc]init];
            realNameVC.orderModel = self.model;
            realNameVC.realNameModel = self.realNameModel;
            realNameVC.modalPresentationStyle = UIModalPresentationOverFullScreen;
            realNameVC.returnAgoBlock = ^(NSString * _Nonnull str) {
                [weakself payStep1];
            };
...
-(void)tipsAlert:(NSString *)content{
...
-(void)payStep1{
    if([self.model.IsOpenFangyiTips isEqualToString:@"1"] && self.model.FangyiTips.length > 0){
        [self tipsAlert:self.model.FangyiTips];
    }else{
