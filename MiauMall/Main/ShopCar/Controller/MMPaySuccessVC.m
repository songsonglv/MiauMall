...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"orderComplete"];
...
-(void)GetOrderType{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.OrderType = [NSString stringWithFormat:@"%@",jsonDic[@"OrderType"]];
            weakself.columnid = [NSString stringWithFormat:@"%@",jsonDic[@"ColumnID"]];
            weakself.PayStatusStr = [NSString stringWithFormat:@"%@",jsonDic[@"PayStatusStr"]];
        }
    } failure:^(NSError *error) {
        dispatch_semaphore_signal(sema);
        NSLog(@"%@",error);
...
-(void)setUI{
    ...
    if([self.columnid isEqualToString:@"860"]){
        [self creatUI];
    }else{
...
-(void)creatUI{
    ...
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake(0, StatusBarHeight + 54, WIDTH, 278)];
    view.backgroundColor = TCUIColorFromRGB(0xffffff);
    [self.view addSubview:view];
    
    UIImageView *iconImg = [[UIImageView alloc]initWithFrame:CGRectMake((WIDTH - 33)/2, 32, 32, 32)];
    iconImg.image = [UIImage imageNamed:@"select_yes"];
    [view addSubview:iconImg];
    
    UILabel *lab = [UILabel publicLab:self.PayStatusStr textColor:TCUIColorFromRGB(0x373737) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Medium" size:21 numberOfLines:0];
    ...
    [btn1 setTitle:[UserDefaultLocationDic valueForKey:@"returnHome"] forState:(UIControlStateNormal)];
    ...
    [btn2 setTitle:[UserDefaultLocationDic valueForKey:@"orderDetails"] forState:(UIControlStateNormal)];
...
-(UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath{
    ...
    if(kind == UICollectionElementKindSectionHeader){
        ...
        UILabel *lab1 = [UILabel publicLab:[NSString stringWithFormat:@"- %@ -",[UserDefaultLocationDic valueForKey:@"mayLike"]] textColor:TCUIColorFromRGB(0x202020) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Medium" size:16 numberOfLines:0];
...
-(void)lookOrder{
    if([self.OrderType isEqualToString:@"充值订单"]){
        [self jumpRouters:@"/secondPages/balance/index"];
    }else{
        if([self.columnid isEqualToString:@"860"]){
            MMDMOrderDetailVC *detailVC = [[MMDMOrderDetailVC alloc]init];
            detailVC.orderID = self.ID;
