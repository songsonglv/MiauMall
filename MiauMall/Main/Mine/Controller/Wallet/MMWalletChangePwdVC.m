...
@interface MMWalletChangePwdVC ()<UITextFieldDelegate>
...
@implementation MMWalletChangePwdVC
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [TalkingDataSDK onPageBegin:@"修改支付密码页面"];
}
...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"changeZfPwd"];
...
-(void)setUI{
    ...
    NSArray *placeArr = @[[UserDefaultLocationDic valueForKey:@"inputSourcePwd"],[UserDefaultLocationDic valueForKey:@"inputSixPwd"],[UserDefaultLocationDic valueForKey:@"inputPwdAgain"],[UserDefaultLocationDic valueForKey:@"inputImgCode"]];
    ...
    for (int i = 0; i < placeArr.count; i++) {
        ...
        UITextField *field1 = [[UITextField alloc]initWithFrame:CGRectMake(77, 23, 220, 20)];
    ...
    [retrieveBt setTitleColor:TCUIColorFromRGB(0xffffff) forState:(UIControlStateNormal)];
    ...
    [retrieveBt mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.top.mas_equalTo(StatusBarHeight + 395);
    ...
    if(self.oldPwdField.text.length > 0 && self.nePwdField.text.length > 0 && self.againPwdField1.text.length > 0 && self.codePwdField.text.length >0){
        if([self.codePwdField.text isEqualToString:self.codeView.changeString]){
            if(self.nePwdField.text.length >= 6 && self.nePwdField.text.length < 20){
                ...
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"sixTwentyPass"]];
            }
        }else{
            [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputImgCode"]];
        }
    }else{
...
-(void)requestSavePwd{
    ...
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"UpdatePaymentPassword"];
    if(self.memberToken){
        [param setValue:self.memberToken forKey:@"membertoken"];
    }
    [param setValue:self.lang forKey:@"lang"];
    [param setValue:self.cry forKey:@"cry"];
    [param setValue:self.oldPwdField.text forKey:@"OldPaymentPassword"];
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    NSLog(@"%@",self.codeView.changeString);
}
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [ZTProgressHUD hide];
    [TalkingDataSDK onPageEnd:@"修改支付密码页面"];
}
