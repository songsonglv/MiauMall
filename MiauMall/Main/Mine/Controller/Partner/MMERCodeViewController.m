...
//            if(weakself.model.Pictures.count > 0){
//                NSDictionary *dic = self.model.Pictures[0];
//                dispatch_queue_t q = dispatch_queue_create("hei", DISPATCH_QUEUE_CONCURRENT);
//                dispatch_async(q, ^{
//                    CGSize size1 = [UIImage getImageSizeWithURL:[NSURL URLWithString:dic[@"Picture"]]];
//                    weakself.hei = WIDTH*(size1.height/size1.width);
//
...
-(void)setUI{
    ...
    for (int i = 0; i < self.model.Pictures.count; i++) {
        ...
        [arr addObject:dic[@"Picture"]];
    }
    
    
    SDCycleScrollView *cycle = [[SDCycleScrollView alloc]initWithFrame:CGRectMake(0, 0, WIDTH, HEIGHT)];
    cycle.imageURLStringsGroup = arr;
    cycle.placeholderImage = [UIImage imageNamed:@"zhanweif"];
...
//    }
    ...
    regCodeLa.frame = CGRectMake((WIDTH - 180)/2, CGRectGetMaxY(codeImage.frame) + 10, 150, 14);
