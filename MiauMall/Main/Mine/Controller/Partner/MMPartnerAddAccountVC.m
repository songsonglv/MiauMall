...
-(void)setUI{
    [ZTProgressHUD hide];
    UILabel *helloLa = [UILabel publicLab:[NSString stringWithFormat:@"%@，",[UserDefaultLocationDic valueForKey:@"ihello"]] textColor:textBlackColor2 textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:25 numberOfLines:0];
    ...
    UILabel *typeLa1 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"itype"] textColor:TCUIColorFromRGB(0x464342) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:17 numberOfLines:0];
    ...
    UILabel *typeLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"selectTxType"] textColor:textBlackColor2 textAlignment:(NSTextAlignmentRight) fontWithName:@"PingFangSC-Regular" size:17 numberOfLines:0];
    ...
    UILabel *lab = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"accountNumber"] textColor:textBlackColor2 textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:17 numberOfLines:0];
    ...
    field.attributedPlaceholder = [[NSAttributedString alloc]initWithString:[UserDefaultLocationDic valueForKey:@"inputtixianAccount"] attributes:@{NSForegroundColorAttributeName:TCUIColorFromRGB(0xb1b0b0)}];
    field.delegate = self;
    ...
    self.view2 = view2;
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"bankAccount"],[UserDefaultLocationDic valueForKey:@"accountName"],[UserDefaultLocationDic valueForKey:@"openBank"]];
    NSArray *placehArr = @[[UserDefaultLocationDic valueForKey:@"inputYouBank"],[UserDefaultLocationDic valueForKey:@"inputYouName"],[UserDefaultLocationDic valueForKey:@"inputOpenBank"]];
    ...
    [addBt setTitle:[UserDefaultLocationDic valueForKey:@"iadd"] forState:(UIControlStateNormal)];
...
-(void)clickAdd{
    if(!self.type){
        [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"selectTxType"]];
    }else{
        if([self.type isEqualToString:@"0"]){
            if(self.accountField.text.length == 0){
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputZfb"]];
            }else{
                [self requestSaveData];
            }
        }else if ([self.type isEqualToString:@"1"]){
            if(self.accountField.text.length == 0){
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputWeChat"]];
            }else{
                [self requestSaveData];
            }
        }else if ([self.type isEqualToString:@"2"]){
            if(self.bankCardField.text.length == 0){
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputTrueBank"]];
            }else if ( self.openCardField.text == 0){
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputYouName"]];
            }else if (self.nameField.text.length == 0){
                [ZTProgressHUD showMessage:[UserDefaultLocationDic valueForKey:@"inputOpenBank"]];
            }else{
                [self requestSaveData];
            }
        }
