...
-(void)setUI{
    ...
    UILabel *titleLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"heHuoRen"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Regular" size:18 numberOfLines:0];
    ...
    UILabel *inviCodeLa = [UILabel publicLab:[NSString stringWithFormat:@"%@%@",[UserDefaultLocationDic valueForKey:@"yqCode"],self.model.memberInfo.RegCode] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:11 numberOfLines:0];
    ...
    UILabel *lab1 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"myYongJing"] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"pingFangSC-Regular" size:11 numberOfLines:0];
    ...
    UILabel *canLa = [UILabel publicLab:[NSString stringWithFormat:@"%@(JPY)",[UserDefaultLocationDic valueForKey:@"ktiXian"]] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:11 numberOfLines:0];
    canLa.preferredMaxLayoutWidth = 200;
    ...
    UILabel *allMoneyLa = [UILabel publicLab:[NSString stringWithFormat:@"%@%@(JPY)",[UserDefaultLocationDic valueForKey:@"cumulativeMoney"],self.model.memberInfo.YongTotal] textColor:TCUIColorFromRGB(0xffffff) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:11 numberOfLines:0];
    ...
    CGSize size = [NSString sizeWithText:[UserDefaultLocationDic valueForKey:@"tiXian"] font:[UIFont fontWithName:@"PingFangSC-Regular" size:15] maxSize:CGSizeMake(MAXFLOAT,15)];
    ...
    UILabel *tiLa = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"teamData"] textColor:TCUIColorFromRGB(0x464342) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:16 numberOfLines:0];
    tiLa.frame = CGRectMake(16, 16, WIDTH - 56, 16);
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        lab2.preferredMaxLayoutWidth = wid;
    ...
    NSArray *arr1 = @[[UserDefaultLocationDic valueForKey:@"teamCovertion"],[UserDefaultLocationDic valueForKey:@"teamRepurchase"]];
    CGFloat wid1 = (WIDTH - 20)/2;
    for (int i = 0; i < arr1.count; i++) {
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(wid1 * i, CGRectGetMaxY(line2.frame),wid1 , 100)];
        [dataView addSubview:view];
        
        CGSize size = [NSString sizeWithText:arr1[i] font:[UIFont fontWithName:@"PingFangSC-Medium" size:13] maxSize:CGSizeMake(85,MAXFLOAT)];
        ...
        lab.preferredMaxLayoutWidth = 93;
        ...
        [textLa mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(18);
            make.top.mas_equalTo(lab.mas_bottom).offset(8);
            make.height.mas_equalTo(11);
        }];
        
        if(i == 0){
            numLa.text = self.model.TeamConversionRate;
            textLa.text = [NSString stringWithFormat:@"%@%@",[UserDefaultLocationDic valueForKey:@"numberUsers"],self.model.TeamConversionCount];
        }else{
            numLa.text = self.model.TeamRepurchaseRate;
            textLa.text = [NSString stringWithFormat:@"%@%@",[UserDefaultLocationDic valueForKey:@"repurchaseNum"],self.model.TeamRepurchaseCount];
        }
    }
    ...
    NSArray *arr2 = @[[UserDefaultLocationDic valueForKey:@"myTeam"],[UserDefaultLocationDic valueForKey:@"distributionRules"],[UserDefaultLocationDic valueForKey:@"distributionOrd"],[UserDefaultLocationDic valueForKey:@"myAccount"]];
    NSArray *imgArr = @[@"team_icon",@"fx_rule_icon",@"fx_order_icon",@"my_account_icon"];
    CGFloat wid3 = (WIDTH - 20)/arr2.count;
    for (int i = 0; i < arr2.count; i++) {
        UIView *view = [[UIView alloc]initWithFrame:CGRectMake(wid3 * i, 0, wid3, 112)];
        [view1 addSubview:view];
        
        UIImageView *iconI = [[UIImageView alloc]initWithFrame:CGRectMake(0, 20, 42, 42)];
        iconI.centerX = wid3/2;
        iconI.image = [UIImage imageNamed:imgArr[i]];
        [view addSubview:iconI];
        
        CGSize size = [NSString sizeWithText:arr2[i] font:[UIFont fontWithName:@"PingFangSC-Regular" size:13] maxSize:CGSizeMake(wid3,MAXFLOAT)];
        UILabel *lab = [UILabel publicLab:arr2[i] textColor:TCUIColorFromRGB(0x36393d) textAlignment:(NSTextAlignmentCenter) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
        lab.frame = CGRectMake(0, 78, wid3, size.height);
        [view addSubview:lab];
        
        UIButton *btn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, wid3, 112)];
        btn.tag = 100 + i;
...
//        [btn addTarget:self action:@selector(clickBtn:) forControlEvents:(UIControlEventTouchUpInside)];
//        btn.frame = CGRectMake(wid3 * i, 20, wid3, 70);
