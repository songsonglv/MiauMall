//
//  MMMyCollectionViewController.m
//  MiauMall
//
...
@interface MMMyCollectionViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation MMMyCollectionViewController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [TalkingDataSDK onPageBegin:@"我的收藏页面"];
...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"myCollect"];
...
- (UISwipeActionsConfiguration *)tableView:(UITableView *)tableView trailingSwipeActionsConfigurationForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIContextualAction *deleteRowAction = [UIContextualAction contextualActionWithStyle:UIContextualActionStyleDestructive title:@"delete" handler:^(UIContextualAction * _Nonnull action, __kindof UIView * _Nonnull sourceView, void (^ _Nonnull completionHandler)(BOOL)) {
        ...
        MMCollectionModel *model = self.dataArr[indexPath.section];
        [weakself deleGoodsCollection:model.ID];
    }];
    deleteRowAction.title = [UserDefaultLocationDic valueForKey:@"cancelColl"];
...
-(void)GetCollectList{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",baseurl,@"GetCollectList"];
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            NSArray *arr = [MMCollectionModel mj_objectArrayWithKeyValuesArray:jsonDic[@"list"]];
            weakself.dataArr = [NSMutableArray arrayWithArray:arr];
            if(arr.count > 0){
                weakself.titleView.titleLa.text = [NSString stringWithFormat:@"%@（%ld）",[UserDefaultLocationDic valueForKey:@"myCollect"],arr.count];
            }else{
                weakself.titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"myCollect"];
            }
...
-(void)deleGoodsCollection:(NSString *)ID{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",BaseUrl1,@"Collection"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [ZTProgressHUD hide];
    [TalkingDataSDK onPageEnd:@"我的收藏页面"];
}
@end
