...
-(HGCategoryView *)cateView{
    if(!_cateView){
        ...
        _cateView.titles = @[[NSString stringWithFormat:@"%@(%@)",[UserDefaultLocationDic valueForKey:@"waitComment"],self.noCount],[UserDefaultLocationDic valueForKey:@"isComment"]];
...
- (void)viewDidLoad {
    ...
    titleView.titleLa.text = [UserDefaultLocationDic valueForKey:@"myEvaluation"];
...
-(void)refreshData1{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.cateView.titles = @[[NSString stringWithFormat:@"%@(%@)",[UserDefaultLocationDic valueForKey:@"waitComment"],weakself.noCount],[UserDefaultLocationDic valueForKey:@"isComment"]];
...
-(void)setUI{
    ...
    UILabel *codeLa = [UILabel publicLab:[NSString stringWithFormat:@"ID:%@",self.memberDic[@"ID"]] textColor:TCUIColorFromRGB(0x231815) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
