...
-(HGCategoryView *)cateView{
    if(!_cateView){
        ...
        _cateView.titles = @[[UserDefaultLocationDic valueForKey:@"iall"],[UserDefaultLocationDic valueForKey:@"pendingPayment"],[UserDefaultLocationDic valueForKey:@"toBeShipped"],[UserDefaultLocationDic valueForKey:@"toReceived"],[UserDefaultLocationDic valueForKey:@"toEvaluated"],[UserDefaultLocationDic valueForKey:@"afterSales"]];
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshData) name:@"refundSuccess1" object:nil];
...
//    [self setUI];
    ...
    self.refundTipPopView.tapGoonBlock = ^(NSString * _Nonnull str) {
        [weakself refundAfter];
    };
    
    NSString *conStr = [UserDefaultLocationDic valueForKey:@"shsqqx"];
    //@"售后申请期限指包裹签收7天之内，此处不包含清关异常、物流原因等导致包裹退回及美容仪保修等情况";
...
-(void)setNoneData{
    LYEmptyView *emptyView = [LYEmptyView emptyActionViewWithImageStr:@"car_none_icon"
                                                                 titleStr:@""
                                                                detailStr:[UserDefaultLocationDic valueForKey:@"noOrders"]
                                                              btnTitleStr:@""
                                                            btnClickBlock:^(){
...
//            [ZTProgressHUD showMessage:@"没有更多"];
    ...
    if(model.EstimatedShipment.length > 0){
        if(model.itemlist.count > 1){
            if([model.isShow isEqualToString:@"1"]){
                return 307 + 124 * model.itemlist.count;
            }else{
                return 307 + 124 * 1;
            }
        ...
        if(model.itemlist.count > 1){
            if([model.isShow isEqualToString:@"1"]){
                return 352 + 124 * model.itemlist.count;
            }else{
                return 352 + 124 * 1;
            }
        }else{
            if([model.ColumnID isEqualToString:@"804"] && [model.AllState isEqualToString:@"7"]){
                return 210 + 124 * model.itemlist.count - 55;
            }else if ([model.ColumnID isEqualToString:@"804"] && [model.AllState isEqualToString:@"0"]){
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    MMOrderListModel *model = self.orderArr[indexPath.row];
    cell.model = model;
    cell.tapShowBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself refreshCellHeight:model];
    };
    ...
    cell.tapKefuBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself clickKefu:model];
    };
    
    cell.tapReceiptBlock = ^(MMOrderListModel * _Nonnull model) {
        KweakSelf(self);
    ...
    cell.tapPayBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself GetPayHandleMoreOrders:model.ID];
    };
    
    cell.tapResultBlock = ^(MMOrderListModel * _Nonnull model) {
        MMAfterSalesResultVC *reasultVC = [[MMAfterSalesResultVC alloc]init];
        reasultVC.orderID = model.ID;
        [weakself.navigationController pushViewController:reasultVC animated:YES];
    };
    
    //退款
    cell.tapRefundBlock = ^(MMOrderListModel * _Nonnull model) {
        weakself.seleModel = model;
        if([model.Processing isEqualToString:@"6"]){
            [weakself requestOrderRefundMoneyTips:model.OrderNumber];
        }else{
            [weakself refundAfter];
        }
    };
    ...
    //查看物流
    cell.tapLogisticBlock = ^(MMOrderListModel * _Nonnull model) {
    ...
    //售后
    cell.tapSaleServiceBlock = ^(MMOrderListModel * _Nonnull model) {
        [[UIApplication sharedApplication].keyWindow addSubview:self.tipsPopView];
        [weakself.tipsPopView showView];
    };
    
    //评价
    cell.tapAssessBlock = ^(MMOrderListModel * _Nonnull model) {
        MMMyAssessViewControllerView *assessVC = [[MMMyAssessViewControllerView alloc]init];
        [weakself.navigationController pushViewController:assessVC animated:YES];
    };
    
    //提醒发货
    cell.tapTxfhBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself tipsOrderID:model.ID];
    };
    
    cell.tapAddressBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself requestOrderAddress:model.ID];
    };
    
    cell.tapAddCartBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself addCar: model.ID];
    };
    
    cell.tapCancleBlock = ^(MMOrderListModel * _Nonnull model) {
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:[UserDefaultLocationDic valueForKey:@"itips"]
        ...
        [alert addAction:[UIAlertAction actionWithTitle:[UserDefaultLocationDic valueForKey:@"idetermine"]
                                                ...
                                                handler:^(UIAlertAction * _Nonnull action) {
            [action setValue:selectColor forKey:@"titleTextColor"];
            [weakself cancleOrderID:model.ID];
        }]];
        [self presentViewController:alert animated:true completion:nil];
    };
    
    cell.tapDeleteBlock = ^(MMOrderListModel * _Nonnull model) {
        [weakself deleteOrder:model.ID];
    };

    return cell;
}

-(void)refreshCellHeight:(MMOrderListModel *)model{
    
    for (int i = 0; i < self.orderArr.count; i++) {
        MMOrderListModel *model1 = self.orderArr[i];
        if([model1.ID isEqualToString:model.ID]){
            [self.orderArr replaceObjectAtIndex:i withObject:model];
        }
    }
...
-(void)clickCenter:(NSString *)type andItemID:(NSString *)ID andModel:(MMOrderListModel *)model{
    if([type isEqualToString:@"0"]){
        if([model.ColumnID isEqualToString:@"804"]){
            KweakSelf(self);
            UIAlertController *alert = [UIAlertController alertControllerWithTitle:[UserDefaultLocationDic valueForKey:@"itips"]
                                                                           message:[UserDefaultLocationDic valueForKey:@"conCleanOrder"]
                                                                    preferredStyle:UIAlertControllerStyleAlert];
...
-(void)clickKefu:(MMOrderListModel *)model{
    MMCustomerServiceVC *customVC = [[MMCustomerServiceVC alloc]init];
    customVC.orderID = model.ID;
...
-(void)refundOrder{
    MMAfterSalesAssessVC *afterAssessVC = [[MMAfterSalesAssessVC alloc]init];
    afterAssessVC.orderId = self.seleModel.ID;
    [self.navigationController pushViewController:afterAssessVC animated:YES];
}
