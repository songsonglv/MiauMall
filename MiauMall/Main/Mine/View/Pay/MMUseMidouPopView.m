...
-(void)creatUI{
    ...
    CGSize size = [NSString sizeWithText:[UserDefaultLocationDic valueForKey:@"termsUse"] font:[UIFont fontWithName:@"PingFangSC-Regular" size:12] maxSize:CGSizeMake(MAXFLOAT,12)];
    UIButton *ruleBt = [[UIButton alloc]init];
    [ruleBt setTitle:[UserDefaultLocationDic valueForKey:@"termsUse"] forState:(UIControlStateNormal)];
    ...
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"noUsePoints"],@"抵扣USD3.89最高可使用865蜜豆",[UserDefaultLocationDic valueForKey:@"customPoints"]];
    for (int i = 0; i < arr.count; i++) {
        ...
        [btn setEnlargeEdgeWithTop:12 right:300 bottom:12 left:20];
        [view addSubview:btn];
        ...
        if(i == 0){
            ...
            [lab rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
                confer.text([UserDefaultLocationDic valueForKey:@"iduction"]).textColor(TCUIColorFromRGB(0x373838));
    ...
    UILabel *lab1 = [UILabel publicLab:[UserDefaultLocationDic valueForKey:@"iuse"] textColor:TCUIColorFromRGB(0x373838) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
    ...
    [lab2 rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text([UserDefaultLocationDic valueForKey:@"midou"]).textColor(TCUIColorFromRGB(0x373838));
    }];
    ...
    tipLa.frame = CGRectMake(46, 218, WIDTH - 60, 12);
...
-(void)setModel:(MMConfirmOrderModel *)model{
    ...
    [self.midouBalanceLa rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text([UserDefaultLocationDic valueForKey:@"mintegral"]).textColor(TCUIColorFromRGB(0x373838)).font([UIFont fontWithName:@"PingFangSC-SemiBold" size:16]);
        confer.text([NSString stringWithFormat:@"（%@：%@）",[UserDefaultLocationDic valueForKey:@"iremaining"],weakself.model.MyIntegral]).textColor(TCUIColorFromRGB(0x373838)).font([UIFont fontWithName:@"PingFangSC-Regular" size:13]);
    }];
    ...
    [self.allMoney rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        ...
        confer.text([NSString stringWithFormat:@"%@%@%@",[UserDefaultLocationDic valueForKey:@"maxUseNum"],weakself.model.MyIntegral,[UserDefaultLocationDic valueForKey:@"midou"]]).textColor(TCUIColorFromRGB(0x373838));
    }];
    
    self.numField.text = _model.MyIntegral;
    self.num = _model.MyIntegral;
    self.tipsLa.text = [NSString stringWithFormat:@"%@%@%@",[UserDefaultLocationDic valueForKey:@"maxCanUseInThisOrder"],_model.MyIntegral,[UserDefaultLocationDic valueForKey:@"miDouNum"]];
}
