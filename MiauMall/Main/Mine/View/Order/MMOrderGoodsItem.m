...
-(void)setUI{
    ...
    for (int i = 0; i < self.goodsArr.count; i++) {
        ...
        numLa.frame = CGRectMake(view.width - 75, 35, 60, 13);
        ...
        //[NSString stringWithFormat:@"%@ %@",[UserDefaultLocationDic valueForKey:@"actualPayment"],model.SalesMoneyShow]
        UILabel *acturaPriceLa = [UILabel publicLab:@"" textColor:redColor2 textAlignment:(NSTextAlignmentRight) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
...
//        acturaPriceLa.hidden = YES;
        ...
        if([model.Processing isEqualToString:@"3"]){
            acturaPriceLa.text = @"已退款";
        }else if ([model.Processing isEqualToString:@"5"]){
            acturaPriceLa.text = @"已取消";
        }else{
            acturaPriceLa.text = @"";
        }
...
-(void)setDic:(NSDictionary *)dic{
    _dic = dic;
    MMOrderListGoodsItemModel *model = [MMOrderListGoodsItemModel mj_objectWithKeyValues:_dic];
