...
-(void)setUI{
    NSArray *arr = @[[UserDefaultLocationDic valueForKey:@"proAmount"],[UserDefaultLocationDic valueForKey:@"idiscount"],[UserDefaultLocationDic valueForKey:@"icoupon"],[UserDefaultLocationDic valueForKey:@"mintegral"],[UserDefaultLocationDic valueForKey:@"proFreight"]];
    for (int i = 0; i < arr.count; i++) {
        ...
        lab.frame = CGRectMake(15, 14, size.width, 13);
        [view addSubview:lab];
...
-(void)setModel:(MMOrderDetailInfoModel *)model{
    ...
    if([_model.ColumnID isEqualToString:@"804"]){
        self.lab1.text = [UserDefaultLocationDic valueForKey:@"depositMoney2"];
    }
    ...
    [self.actualMoneyLa rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
        confer.text([UserDefaultLocationDic valueForKey:@"actualPayment"]).textColor(TCUIColorFromRGB(0x242424)).font([UIFont fontWithName:@"PingfangSC-Medium" size:14]);
