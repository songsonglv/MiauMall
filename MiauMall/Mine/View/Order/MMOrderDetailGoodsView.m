...
-(void)setUI{
    self.backgroundColor = TCUIColorFromRGB(0xffffff);
    UILabel *lab = [UILabel publicLab:@"商品信息" textColor:TCUIColorFromRGB(0x383838) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:16 numberOfLines:0];
    lab.frame = CGRectMake(14, 16, 80, 16);
    [self addSubview:lab];
    ...
    for (int i = 0; i < self.arr.count; i++) {
        ...
        UILabel *acturaPriceLa = [UILabel publicLab:[NSString stringWithFormat:@"实付款%@",model.SalesMoneyShow] textColor:TCUIColorFromRGB(0x0b0b0b) textAlignment:(NSTextAlignmentRight) fontWithName:@"PingFangSC-SemiBold" size:13 numberOfLines:0];
        ...
        [acturaPriceLa rz_colorfulConfer:^(RZColorfulConferrer * _Nonnull confer) {
            confer.text(@"实付款").textColor(TCUIColorFromRGB(0x0b0b0b)).font([UIFont systemFontOfSize:13]);
...
//        }];
        ...
        [addBt setTitle:@"加入购物车" forState:(UIControlStateNormal)];
