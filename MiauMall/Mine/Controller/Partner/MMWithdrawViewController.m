...
-(void)requestSelectData{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.banlanceLa.text = [NSString stringWithFormat:@"当前可提现余额JPY%@",weakself.selectModel.YongBalance];
            weakself.feeLa.text = [NSString stringWithFormat:@"(提现手续费%@JPY",weakself.selectModel.ExtractFees];
...
-(void)setUI{
    ...
    UITextField *field = [[UITextField alloc]initWithFrame:CGRectMake(68, StatusBarHeight + 205, WIDTH - 90, 48)];
//    field.backgroundColor = TCUIColorFromRGB(0xffffff);
    field.delegate = self;
    field.attributedPlaceholder = [[NSAttributedString alloc]initWithString:@"请输入提现金额" attributes:@{NSForegroundColorAttributeName:TCUIColorFromRGB(0xb1b0b0)}];
    field.font = [UIFont fontWithName:@"PingFangSC-SemiBold" size:40];
    ...
    UILabel *balanceLa = [UILabel publicLab:@"当前可提现余额," textColor:TCUIColorFromRGB(0x5a5a5a) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
    ...
    [allBt setTitle:@"全部提现" forState:(UIControlStateNormal)];
    ...
    UILabel *lab2 = [UILabel publicLab:@"最低提现金额" textColor:TCUIColorFromRGB(0x727272) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:12 numberOfLines:0];
    lab2.preferredMaxLayoutWidth = 120;
    ...
    UILabel *lab3 = [UILabel publicLab:@"提现提醒" textColor:TCUIColorFromRGB(0xff625b) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
    ...
    UILabel *lab4 = [UILabel publicLab:@"每笔提现金额将收取部分服务费+手续费 \n具体以实际到账金额为准" textColor:TCUIColorFromRGB(0xa5a5a5) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:12 numberOfLines:0];
    lab4.preferredMaxLayoutWidth = 215;
...
-(void)clickWithdraw{
    ...
    if(self.moneyField.text.length > 0 && money > 0){
        ...
        [ZTProgressHUD showMessage:@"请输入正确的提现金额"];
    }
