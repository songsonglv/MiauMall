...
-(UIView *)view1{
    ...
    if(!_view1){
        ...
        UILabel *lab1 = [UILabel publicLab:@"上传商品图片" textColor:TCUIColorFromRGB(0x3c3c3c) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        lab1.frame = CGRectMake(12, 1, 90, 14);
        [conV addSubview:lab1];
        
        ACSelectMediaView *mediaView = [[ACSelectMediaView alloc]initWithFrame:CGRectMake(12,30, 85, 81) andIsEnter:@"wish"];
        ...
        UILabel *lab2 = [UILabel publicLab:@"商品名称" textColor:TCUIColorFromRGB(0x3c3c3c) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        lab2.frame = CGRectMake(12, 120, 90, 14);
        ...
        UILabel *lab3 = [UILabel publicLab:@"商品描述" textColor:TCUIColorFromRGB(0x3c3c3c) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
        lab3.frame = CGRectMake(12, CGRectGetMaxY(teV.frame) + 21, 90, 14);
        ...
        UILabel *textLa = [UILabel publicLab:@"品牌、商品名称、型号、尺寸或正在销售此商品的app名称。" textColor:TCUIColorFromRGB(0xc5c5c5) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
        ...
        UILabel *lab4 = [UILabel publicLab:@"我的心愿" textColor:TCUIColorFromRGB(0x3c3c3c) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-SemiBold" size:14 numberOfLines:0];
...
-(UIView *)bottomV{
    if(!_bottomV){
        ...
        [uploadBt setTitle:@"提交我的心愿单" forState:(UIControlStateNormal)];
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            if(arr.count > 0){
                ...
                [ZTProgressHUD showMessage:@"没有更多数据!"];
...
-(void)setUI{
    ...
    MMWishTopView *changeView  = [[MMWishTopView alloc]initWithFrame:CGRectMake((WIDTH - 140)/2, StatusBarHeight + 13, 140, 25)];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.goShopBlock = ^(NSString * _Nonnull indexStr) {
        [ZTProgressHUD showMessage:indexStr];
    };
    return cell;
...
-(void)clickUpload{
    KweakSelf(self);
    if(!self.imageUrl){
        [ZTProgressHUD showMessage:@"请添加图片"];
    }else if (self.goodsNameField.text.length == 0){
        [ZTProgressHUD showMessage:@"请添加商品名称"];
    }else if (self.desView.text.length == 0){
        [ZTProgressHUD showMessage:@"请添加商品描述"];
    }else{
        NSMutableDictionary *param = [[NSMutableDictionary alloc]init];
        ...
        [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
            ...
            if([code isEqualToString:@"1"]){
                [weakself requestMyWishData];
                [ZTProgressHUD showMessage:@"心愿单已提交，请等待审核"];
