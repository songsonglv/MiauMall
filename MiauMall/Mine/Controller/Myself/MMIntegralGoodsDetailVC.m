...
@interface MMIntegralGoodsDetailVC ()<SDCycleScrollViewDelegate>
...
-(MMGoodsDetailBottomView *)bottomView{
    if(!_bottomView){
        ...
        [_bottomView.exchangeBt setTitle:@"立即兑换" forState:(UIControlStateNormal)];
...
-(void)requestData{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            dispatch_async(q, ^{
                CGFloat hei = 0;
                for (int i = 0; i < weakself.model.ContsStr.count; i++) {
                    NSString *url = weakself.model.ContsStr[i];
...
//    [self.view addSubview:moreBt];
    ...
    UILabel *integralLa = [UILabel publicLab:[NSString stringWithFormat:@"%@积分",self.model.IntegralFull] textColor:redColor2 textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-SemiBold" size:19 numberOfLines:0];
    integralLa.frame = CGRectMake(24, CGRectGetMaxY(nameLa.frame) + 14, WIDTH - 48, 20);
    [scrollView addSubview:integralLa];
    
    UILabel *stockLa = [UILabel publicLab:[NSString stringWithFormat:@"库存：%@",self.model.Number] textColor:TCUIColorFromRGB(0x272727) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
    ...
    UILabel *salesLa = [UILabel publicLab:[NSString stringWithFormat:@"已兑换：%@",self.model.Sales] textColor:TCUIColorFromRGB(0x272727) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:13 numberOfLines:0];
    ...
    float Hei = 0;
    for (int i = 0; i < self.model.ContsStr.count; i++) {
        NSString *heistr = self.goodsImageHArr[i];
        NSString *url = self.model.ContsStr[i];
        CGFloat hei = [heistr floatValue];
        UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake(10, CGRectGetMaxY(stockLa.frame) + 20 + Hei , WIDTH - 20, hei)];
        [image sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"zhanweif"]];
        [scrollView addSubview:image];
        Hei += hei;
    }
    scrollView.contentSize = CGSizeMake(WIDTH, CGRectGetMaxY(stockLa.frame) + 20 + Hei);
}
...
-(void)buyNow{
    ...
    if(integralBalance < price){
        ...
        [ZTProgressHUD showMessage:@"生成订单"];
    }
}
