...
-(UIView *)bottomView{
    if(!_bottomView){
        ...
        UILabel *allLa = [UILabel publicLab:@"全选" textColor:TCUIColorFromRGB(0x333333) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Medium" size:14 numberOfLines:0];
...
-(void)GetCollectList{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            if(arr.count > 0){
                weakself.titleView.titleLa.text = [NSString stringWithFormat:@"我的收藏（%ld）",arr.count];
            }else{
                weakself.titleView.titleLa.text = @"我的收藏";
            }
