...
-(void)GetCollectList{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            if(arr.count > 0){
                weakself.titleView.titleLa.text = [NSString stringWithFormat:@"我的足迹（%ld）",arr.count];
            }else{
                weakself.titleView.titleLa.text = @"我的足迹";
            }
...
-(void)refreshData{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",BaseUrl1,@"GetBrowseRecord"];
...
-(void)deleGoodsCollection:(NSString *)ID{
    KweakSelf(self);
    NSString *url = [NSString stringWithFormat:@"%s?t=%@",BaseUrl1,@"BrowseRecordDel"];
