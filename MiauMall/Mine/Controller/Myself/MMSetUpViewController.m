...
-(void)requestData{
    ...
    [ZTNetworking FormPostRequestUrl:url RequestPatams:param ssuccess:^(NSString *jsonStr, NSDictionary *jsonDic) {
        ...
        if([code isEqualToString:@"1"]){
            ...
            weakself.IDLabel.text = [NSString stringWithFormat:@"您的ID：%@",weakself.model.memberInfo.ID];
        }else{
...
-(void)setUI{
    ...
    NSArray *arr = @[@"我的地址",@"账号管理",@"密码重置",@"消息设置",@"地区和语言",@"版本号"];
    ...
    [outLoginBt setTitle:@"退出登录" forState:(UIControlStateNormal)];
