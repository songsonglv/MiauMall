...
-(void)creatUI{
    UILabel *titleLa = [UILabel publicLab:@"最近搜索" textColor:TCUIColorFromRGB(0x959595) textAlignment:(NSTextAlignmentLeft) fontWithName:@"PingFangSC-Regular" size:14 numberOfLines:0];
...
-(void)setDataArr:(NSArray *)dataArr{
    ...
    for(UILabel *mylabel in [self subviews])
    {
        if ([mylabel isKindOfClass:[UILabel class]]) {
            if([mylabel.text isEqualToString:@"最近搜索"]){
...
- (void)tagDidCLick:(UITapGestureRecognizer *)tap
{
    ...
    self.tapSearchBlcok(label.text);
}
